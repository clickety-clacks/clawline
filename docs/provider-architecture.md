...
- **Streaming**: no separate `partial` type. Use `message` with `streaming: true` and full `content` so far. A final `message` with `streaming: false` must include full `content`. If the originating socket disconnects before another socket authenticates (session takeover), partial text is lost—there is no replay of `streaming: true` events.
