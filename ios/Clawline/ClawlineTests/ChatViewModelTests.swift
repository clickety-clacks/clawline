...
struct ChatViewModelTests {
    ...
    @Test("Selected session provider status reaches its footer")
    @MainActor
    func selectedSessionProviderStatusReachesFooter() async throws {
        resetChatPersistence()
        let auth = TestAuthManager()
        auth.storeCredentials(token: "jwt", userId: "user")
        let selectedSessionKey = personalSessionKey
        ...
        HTTPStubURLProtocol.requestHandler = { request in
            ...
            case "/api/session-status":
                ...
                #expect(queryItems?.first(where: { $0.name == "sessionKey" })?.value == selectedSessionKey)
        ...
        let chatService = ProviderChatService(
            connector: MockWebSocketConnector(client: mockSocket),
            deviceId: "device_selected_footer",
            baseURLProvider: { baseURL },
        ...
        let viewModel = ChatViewModel(
            ...
            chatService: chatService,
            settings: SettingsManager(),
            device: ProviderStatusTestDevice(),
            uploadService: ProviderStatusTestUploadService(),
            toastManager: ToastManager(),
            salientHighlightService: SalientHighlightService()
        )
        defer { viewModel.prepareForReplacement() }
        ...
        await viewModel.activate(origin: "test.selectedSessionProviderStatusReachesFooter")

        for _ in 0..<100 {
            if viewModel.uiSelectedSessionKey == selectedSessionKey,
               viewModel.sessionStatus(for: selectedSessionKey)?.display.model == "claude-sonnet-4.6" {
                break
            }
            try await Task.sleep(for: .milliseconds(20))
        }

        let status = viewModel.sessionStatus(for: selectedSessionKey)
        #expect(viewModel.uiSelectedSessionKey == selectedSessionKey)
        #expect(status?.sessionKey == selectedSessionKey)
        #expect(
            SessionMetadataFooterCell.footerText(for: status)
                == "claude-sonnet-4.6  ·  Thinking high  ·  Fast on  ·  OAUTH"
        )
    }
...
private struct ProviderStatusTestDevice: DeviceIdentifying {
    let deviceId = "device_selected_footer"
}

private final class ProviderStatusTestUploadService: UploadServicing {
