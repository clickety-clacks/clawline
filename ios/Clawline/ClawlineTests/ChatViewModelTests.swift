...
struct ChatViewModelTests {
    ...
    @Test("Harness options use per-session capability and never fall back to org-options")
    @MainActor
    func harnessOptionsUseSessionStatusCapabilityOnly() async throws {
        resetChatPersistence()
        let auth = TestAuthManager()
        auth.storeCredentials(token: "jwt", userId: "user")
        let chatService = TestChatService()
        ...
        chatService.orgOptions = OrgOptions(
            harnesses: ["org-only"], models: [:], hosts: [], archetypes: []
        )
        chatService.sessionStatusBySessionKey[personalSessionKey] = makeSessionStatus(
            sessionKey: personalSessionKey,
            state: .idle,
        ...
        await viewModel.activate(origin: "test.harnessCapabilityOptions")
    ...
    @Test("setHarness refuses stale or unavailable status before posting")
    @MainActor
    func setHarnessRefusesStaleOrUnavailableStatusBeforePosting() async throws {
        resetChatPersistence()
        let auth = TestAuthManager()
        auth.storeCredentials(token: "jwt", userId: "user")
        let chatService = TestChatService()
    ...
    @Test("setHarness refuses removed disabled or empty options")
    @MainActor
    func setHarnessRefusesRemovedDisabledOrEmptyOptions() async throws {
        resetChatPersistence()
        let auth = TestAuthManager()
        auth.storeCredentials(token: "jwt", userId: "user")
        let chatService = TestChatService()
        ...
        func refreshStatus(_ status: SessionStatus, id: String) async throws {
            chatService.sessionStatusBySessionKey[personalSessionKey] = status
            let fetchCount = chatService.fetchSessionStatusCallCount
            let payload = #"{"type":"message","id":"\#(id)","role":"assistant","content":"noop","timestamp":1700000000004,"streaming":false,"sessionKey":"\#(personalSessionKey)","attachments":[]}"#
            chatService.emitLifecycleEvent(.init(epoch: 1, payload: .serverMessage(data: Data(payload.utf8))))
            for _ in 0..<50 {
                if chatService.fetchSessionStatusCallCount > fetchCount,
                   !viewModel.isSessionStatusStale(for: personalSessionKey) {
                    break
                }
                try await Task.sleep(for: .milliseconds(20))
            }
        ...
        #expect(viewModel.orgOptionsHarnesses == ["codex"])
        #expect(viewModel.unavailableSessionControlReason(
            .setHarness,
            sessionKey: personalSessionKey,
            value: "claude"
        ...
        #expect(viewModel.orgOptionsHarnesses.isEmpty)
        #expect(viewModel.unavailableSessionControlReason(
            .setHarness,
            sessionKey: personalSessionKey,
            value: "claude"
        ...
        capabilities: .init(
            ...
            setHarness: harnessOptions.map { options in
                .init(
                    ...
                    reason: nil,
                    options: options.map {
                        .init(title: $0, value: $0, enabled: harnessOptionEnabled[$0] ?? true)
                    }
                )
            },
